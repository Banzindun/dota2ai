...
public class HeroAttackingDS extends AttackingDS{
	 ...
	 */
	private static final Logger logger = LogManager.getLogger(HeroAttackingDS.class);
	...
	public void createDecisions() {
		for (int i = 0; i < Constants.MAX_HEROES; i++) {
			AttackHeroDecision decision = (AttackHeroDecision) DecisionBuilder.build()
					...
					// I will should look only to friendly_threats layer for that
					.addConsideration(
							new ConsiderThreat(),
	...
	private void updateAttackHeroDecisions(ExtendedBotContext botContext, LinkedList<Hero> enemyHeroes) {
		int i = 0;
