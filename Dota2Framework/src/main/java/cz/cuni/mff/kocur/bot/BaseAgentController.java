...
/**
 * Abstract class that implements some of the methods that can be used by sample Controller. 
 * Also unregisters logger for root class of registered bot. 
 * Defines and handles hero id and creates hero information. 
 * Implemented methods are for example Attack, MoveTo, GrabAll, CastASpell
 * @author Banzindun
 *
 */
public abstract class BaseAgentController implements Controllable, AgentController, ConfigurationChangeListener{
	...
	/**
	 * Team context of this bot. Contains reference to other heroes etc. on the same team. 
	 */
	...
	/**
	 * Reference to actual hero object, that is the hero that comes through updates etc. 
	 */
	...
	/**
	 * This is a command, that can be used by any controller to move to some location.
	 * This will be extended by BaseBotController. 
	 * @param l Location where we want to move. The location can have id or x, y set in it. If only id is set, we will move to entity with that id. If only location is set, we will move to that location.
	...
	/**
	 * This is a command that can be used by extending controller to issue a command to pickup a rune. 
	 * @param b The rune. 
	 * @return Returns command to come closer to rune or to pick it up if standing too close.
	 */
	...
	/**
	 * This is a command that can be used by extending controller to issue a command to use a shrine. 
	...
	/**
	 * This is a command that can be used by extending controller to issue a command to buy a item given by its name.
	 * @param string Name of the item to buy. 
	...
	/**
 * This is a command that can be used by extending controller to issue a command to sell a item on given slot. 
 * @param slot Slot that contains the item that should be sold.
 ...
 */
	...
	/**
	 * This is a command that can be used by extending controller to issue a command to sell item, given its name. The item is found using the name. 
	 * @param itemName Name of the item to be sold. 
	...
	/**
	 * This is a command that can be used by extending controller to issue a command that casts a spell at index at given location.
	 * @param l Location with id or x, y coordinates. If only id given, the spell will be cast on target with given id. Else the spell will be cast on given location, if it is not a passive spell. 
	 * @param index Index of the spell. 
	...
	/**
	 * This is a command that can be used by extending controller to issue a command to attack a entity.
	 * @param e Location, that has an id set. 
	...
	/**
	 * This is a command that can be used by extending controller to issue a command to grab all items. This should be called if you are close to the base shop.
