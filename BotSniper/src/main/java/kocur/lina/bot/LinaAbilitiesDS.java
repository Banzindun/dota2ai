...
import kocur.sniper.bot.CastAssasinate;
import kocur.sniper.bot.CastShrapnel;
import kocur.sniper.bot.SniperAbilitiesDS;

public class LinaAbilitiesDS extends DecisionSet{
	
	private static final Logger logger = LogManager.getLogger(LinaAbilitiesDS.class);
	...
	public void createDecisions() {
		...
		Decision dragonSlave = DecisionBuilder.build()
				.setDecision(new CastDragonSalve())
				.setName("CastDragonSlave")
				.setBonusFactor(10.0)
				.addConsideration(
						new ConsiderSourceMana(),
						new PolynomialFunction(-1.8, 2, 1, 0.9))
				.addConsideration(
						new ConsiderCooldown(),
						new BinaryFunction(0.5, -1, 0.01, 0.5))
				// Value will be count of targets I am able to hit:
				// 4 is good, 2 is not good enough -> we want to cast at 4/5 targets if possible
				.addConsideration(new ConsiderTargetValue(),
						new PolynomialFunction(-2.7, 2, 1, 1))
		...
		// Decision to cast dragon salve
		/*Decision dragonSalveOnHeroNearby = DecisionBuilder.build()
				.setDecision(new CastDragonSalve())
				.setName("CastDragonSalveOnNearbyHero")
				.setBonusFactor(7.0) // Smaller factor .. for now
				.addConsideration(
						new ConsiderSourceMana(),
						new PolynomialFunction(-1.8, 2, 1, 0.9))
				.addConsideration(
		...
		// Decision to cast dragon salve
		Decision lightArray = DecisionBuilder.build()
				.setDecision(new CastLightArray())
				.setName("CastLightArray")
				.setBonusFactor(10.0)
				.addConsideration(
						new ConsiderSourceMana(),
						new PolynomialFunction(-1.8, 2, 1, 0.9))
				...
				.addConsideration(new ConsiderTargetHealth(),
						new PolynomialFunction(-2.5, 2, -0.1, 1.6))
				.setEvaluator(new DecisionScoreEvaluator())
				.get();
		...
		Decision lagunaBlade = DecisionBuilder.build()
				.setDecision(new CastLagunaBlade())
				.setName("CastLagunaBlade")
				.setBonusFactor(10.0)
				.addConsideration(
						new ConsiderSourceMana(),
						new PolynomialFunction(-1.8, 2, 1, 0.9))
				...
				.addConsideration(new ConsiderTargetHealth(),
						new PolynomialFunction(-2.7, 2, -0.3, 1.4))
				.addDoubleParameter(Consideration.PARAM_RANGE_MIN, 0)
				.addDoubleParameter(Consideration.PARAM_RANGE_MAX, 6)
				.setEvaluator(new DecisionScoreEvaluator())
				.get();
