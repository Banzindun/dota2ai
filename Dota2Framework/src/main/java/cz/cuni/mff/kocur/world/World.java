...
public class World {
    ...
    /**
     * 
     * @param id Id of the entity we want to find.
    ...
    public int searchIdByName( String name ) {
        final Entry<Integer, BaseEntity> e = entities.entrySet().stream().filter( p -> p.getValue().getName().equals( name ) ).findAny().orElse( null );
        if (e == null) {
            return -1;
        }
        else {
     ...
     * @param l Location. Center of our radius.
     * @param dist Radius.
     * @return Returns list of entities inside the radius around the passed location.
     */
	public List<BaseEntity> findEntitiesInRadius(Location l, double dist) {
		return entities.values().stream()
				.filter( p -> GridBase.distance(p, l) < dist && GridBase.distance(p, l) != 0)
	 ...
	 * @param e1 Id of the first entity.
	 * @param e2 If of the second entity.
    ...
    /**
	 * Updates this world.
	 * Updates puts all entities to world representation.
	 * @param u WorldUpdate - world that we are updating from.
	 */
	...
	/**
	 * Cleans up the world, but doesn't alert entities that they are dying. </hr>
	 * This removes entities that should disappear.
	 * @return Returns number of entities that were removed.
	 */
