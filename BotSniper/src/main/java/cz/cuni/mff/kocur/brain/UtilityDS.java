...
public class UtilityDS extends DecisionSet{
	...
	private void createDecisions() {
		Decision useHealingPotion = DecisionBuilder.build()
				.setDecision(new DecisionUsePotion("item_flask"))
				.setName("UseHealingPotion")
				.setBonusFactor(15)
				...
				.addConsideration(
						...
						new LinearFunction(-4, 1, 0.1, 1.5))
				.addConsideration(new ConsiderItemPresent(),
						new BinaryFunction(0.5, -1, (double) Inventory.inventorySlotMax/(Inventory.stashSlotMax+1), 0.5))
		...
		Decision useManaPotion = DecisionBuilder.build()
				.setDecision(new DecisionUsePotion("item_clarity"))
				.setName("UseManaPotion")
				.setBonusFactor(15)
				.addConsideration(new ConsiderTimePassed(),
						new LinearFunction(2,1,0.6,0))
				.addDoubleParameter(Consideration.PARAM_RANGE_MAX, 20) // After 10 seconds 1.0
				.addConsideration(
						...
						new PolynomialFunction(2, -10, -1, -0.02))
				.addConsideration(new ConsiderItemPresent(),
						new BinaryFunction(0.5, -1, (double) Inventory.inventorySlotMax/(Inventory.stashSlotMax+1), 0.5))
				.addDoubleParameter(Consideration.PARAM_RANGE_MIN, 0)
				.addDoubleParameter(Consideration.PARAM_RANGE_MAX, Inventory.stashSlotMax + 1)
		...
		Decision pickupRune = DecisionBuilder.build()
				.setDecision(new PickupRuneDecision())
				.setName("PickupRune")
				.setBonusFactor(5)
				...
				.addDoubleParameter(Consideration.PARAM_RANGE_MIN, 0)
				.addDoubleParameter(Consideration.PARAM_RANGE_MAX, 400)				
				.setEvaluator(new DecisionScoreEvaluator())
				.get();
