...
public class LoggingTabConfiguration extends JPanel {
	...
	// LogManager.getLogger(LoggingTabConfiguration.class.getName());
	...
	/**
	 * Private class that holds configuration of fields for logs. Log should be
	 * filtered so only filters that are demanded are displayed.
	 * 
	 * @author Banzindun
	 *
	 */
	private class Fields {
		...
		/**
		 * Patterns that correspond to {@link #fields}, that are later merged to create
		 * PatternLayout.
		 */
		 ...
		 * 
		 * @param d
		 *            Field of booleans that specify what fields are logged.
		 */
		public void setDisplayed(boolean[] d) {
			if (Arrays.equals(display, d))
				return;
			else {
				display = d;
				buildLayout();
		...
		private void buildLayout() {
			...
			if (display == null || display.length != fields.length)
				return;
			...
			layout = PatternLayout.newBuilder().withPattern(pattern.toString()).build();
		}
	...
	/**
	 * Array of check boxes that are checked when user doesn't want to display the
	 * checked fields. Initially everything is displayed.
	 */
	...
	/**
	 * Selected if user wants to display logs with level that is more specific than
	 * the selected one.
	 */
	...
	/**
	 * Setups the configuration. Initializes constraints and adds fields to
	 * configuration.
	 */
	public void setup() {
		...
		GridBagConstraints gbc = ConstraintsBuilder.build().gridy(0).gridx(0).fill(GridBagConstraints.HORIZONTAL)
				.anchor(GridBagConstraints.EAST).insets(2, 2, 2, 2).get();
	...
	/**
	 * Initializes display field. That is a field that lets user select what he
	 * wants to log.
	 * 
	 * @return JPanel that contains all the necessary CheckBoxes etc. to let the
	 *         user configure all the fields.
	 */
	private JPanel initDisplayField() {
		...
		GridBagConstraints gbc = ConstraintsBuilder.build().fill(GridBagConstraints.BOTH)
				.anchor(GridBagConstraints.NORTH).gridy(0).gridx(0).weightx(1).insets(2, 5, 2, 5).get();
	...
	/**
	 * Updates display field (after set). Selects already selected fields.
	 */
	 ...
	 * 
	 * @return JPanel that contains all the options, so that user can filter the
	 *         logs by level.
	 */
	private JPanel initFilterField() {
		...
		GridBagConstraints gbc = ConstraintsBuilder.build().fill(GridBagConstraints.BOTH)
				.anchor(GridBagConstraints.NORTH).gridy(0).gridx(0).insets(2, 5, 0, 5).get();
	...
	private void updateFilterFields() {
		if (filterLevel != null)
			levelJComboBox.setSelectedItem(filterLevel);
	...
	/**
	 * Updates save to file - adds button that will stop the output stream if
	 * necessary and name of file.
	 */
	private void updateSaveToFile() {
		...
		if (savingToFile) {
			...
			// Initialize constraints
			GridBagConstraints gbc = ConstraintsBuilder.build().weightx(1).insets(2, 5, 2, 5)
					.anchor(GridBagConstraints.WEST).fill(GridBagConstraints.EAST).get();
	...
	/**
	 * 
	 * @return Returns PatternLayout given by {@link #displayedFields} selected by
	 *         user.
	 */
