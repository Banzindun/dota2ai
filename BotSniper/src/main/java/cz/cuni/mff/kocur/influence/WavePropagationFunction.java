...
public class WavePropagationFunction<T extends Location> implements PropagationFunction<T> {
	...
	/**
	 * Neighbors, that the point can have. We will use them to spread to other tiles.
	 */
	...
	/**
	 * This propagates a passed influence layer. In WPF we spread in a wave-like manner. We decrease distance with each step and we spread the influence accordingly.
	 */
	@Override
	public void propagate(InfluenceLayer l, T e) {
		...
		// Spread the influence. Stop on empty queue (that means we have spread to the distance we have needed).
		while (queue.size() > 0) {
			distance++;
			spread(l);
		}
