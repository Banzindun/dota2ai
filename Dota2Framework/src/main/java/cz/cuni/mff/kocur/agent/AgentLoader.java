...
/**
 * The purpose of this class is to load agents from framework configuration. The
 * agent configurations are stored inside framework configuration, after they
 * are loaded.
 * 
 * @author Banzindun
 *
...
public class AgentLoader {
	 ...
	 */
	private static Logger logger = LogManager.getLogger(AgentLoader.class.getName());
	...
	/**
	 * Loads bots from configurations that are supplied using the framework (and
	 ...
	 * @throws LoadingError
	 *             If some of the bots could not be loaded.
	 */
	 ...
	 * 
	 * @param c
	 *            BotConfiguration that represents the bot.
	...
	/**
	 * Loads an agent from configuration and creates a new BaseAgentController
	 * instance out of it.
	 * 
	 * @param c
	 *            Agent's configuration.
	 ...
	 * @throws LoadingError
	 *             Thrown if something went wrong during agent loading.
	 */
	private static BaseAgentController loadAgentController(HeroConfiguration c) throws LoadingError {
		...
		if (cl == null)
			throw new LoadingError("No class in configuration: " + c.getName());

		// Load the class and construct the agent by passing agent's configuration to
		// it.
		try {
	 ...
	 * 
	 * @param className
	 *            Class path we want to register the appender for.
	 */
	...
	/**
	 * Returns true if the bot can be loaded from supplied configuration. This can
	 * be used for testing the configuration.
	 * 
	 * @param c
	 *            BotConfiguration of the loaded bot.
