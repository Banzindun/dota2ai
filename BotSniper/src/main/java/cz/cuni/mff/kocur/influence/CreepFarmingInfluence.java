...
public class CreepFarmingInfluence extends WavePropagationWithContext<Creep> {
	/**
	 * Logger.
	 */
	private Logger logger = LogManager.getLogger(CreepFarmingInfluence.class);
	...
	/**
	 * We will multiply the result by some coefficient calculated from creep's health.
	 */
	 ...
	 */
	public CreepFarmingInfluence(AgentContext context) {
		super(context);
	}
	...
	public void propagate(InfluenceLayer l, Creep e) {
		...
		coeff = 1-(double) creep.getHealth()/creep.getMaxHealth();
	...
	protected void calculateInfluence(InfluenceLayer l, PropagationPoint point) {
		...
		if (context.getMyTeam() != creep.getTeam()) {
	 ...
	 * @param point Point for which the influence should be calculated.
	...
	private double calculateCreepsInfluence(PropagationPoint point) {
		...
		if (distanceTileToBase > maxDistanceToBase || distanceTileToBase < minDistanceToBase) // This tile is tilted towards enemy's base.
			return 0;
