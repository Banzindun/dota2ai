...
public class SniperAbilitiesDS extends DecisionSet{
	...
	private static final Logger logger = LogManager.getLogger(SniperAbilitiesDS.class);
	...
	public void createDecisions() {
		...
		Decision shrapnel = DecisionBuilder.build()
				.setDecision(new CastShrapnel())
				.setName("CastShrapnel")
				.setBonusFactor(10.0)
				.addConsideration(
		...
		Decision asssasinate = DecisionBuilder.build()
				.setDecision(new CastAssasinate())
				.setName("CastAssasinate")
				.setBonusFactor(10.0)
				.addConsideration(
						new ConsiderSourceMana(),
						new PolynomialFunction(-1.8, 2, 1, 0.9))
				...
				.addDoubleParameter(Consideration.PARAM_RANGE_MIN, 0)
				.addDoubleParameter(Consideration.PARAM_RANGE_MAX, 6)
				// And finally we should be casting it only on targets that are far away (say 800+)
				.addConsideration(
						new ConsiderDistanceToTarget(),
						new LinearFunction(9.3, 1, 0.95, 1.1))
				.addDoubleParameter(Consideration.PARAM_RANGE_MIN, 0)
