...
/**
 * Class that represents radiant threats. We want to spread influence from every radiant entity, 
 * that might be dangerous (towers, creeps etc.)  And we spread influence from radiant fort.
 * The main goal of this class is to capture radiant's part of the game, so it can be analysed 
 * later. 
 * @author kocur
 *
 */
public class RadiantThreatLayer extends InfluenceLayer{
	...
	public void createParams() {
		...
		PolynomialAttackRangePropagationFunction<Tower> towerP =
				new PolynomialAttackRangePropagationFunction<Tower>();
		towerP.setMaxInfluence(1.0);
		towerP.setPower(2.0);		
		
		params = ParamsBuilder.build()
				.createEntityParameter(Tower.class, towerP, Team.RADIANT)
				.createInterestParameter(Fort.class, new FortPropagation<Fort>(1, w/resolution))
				.get(); 
